...
def clean_reply(text):
    # Remove reasoning blocks generated by Qwen
...
VAGUE_INPUT_RE = re.compile(
    ...
    r"|just browsing|just checking|just curious|just saying|not really|not sure|not now|maybe later|later|bye|goodbye|ttyl|gotta go|😂|👍|🙏)(?:\s+then)?[.!?,'\\s😂👍🙏]*$",
    re.IGNORECASE
)
...
def build_persona_response(user_question, chat_history):
    ...
    # Python-level gibberish interceptor (catches 'h', 'gvh', etc before LLM sees them)
    is_short = len(sanitized) > 0 and len(sanitized) <= 2 and sanitized.lower() not in ["hi", "ok", "no", "yo", "ya", "me"]
    is_consonants = re.match(r"^[^aeiouy0-9\s]+$", sanitized, re.IGNORECASE)
    # Also catch common keyboard smashes that have vowels but aren't words
    is_smash = sanitized.lower() in ["hjuioi", "asdf", "qwer", "zxcv"]

    if is_short or is_consonants or is_smash:
        gibberish_replies = ["I didn't quite catch that.", "Come again?", "Not sure what you mean there.", "Huh?"]
        return random.choice(gibberish_replies), random.sample(INTENT_SUGGESTIONS["general"], 3)
